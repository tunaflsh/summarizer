...
def translate(text, source_language, target_language, model='gpt-3.5-turbo', checkpoint='translator.pkl', **kwargs):
    model = Model(model, checkpoint, **kwargs)
    messages = [
        {'role': 'system', 'content': prompt.TRANSLATE.format(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Translate a text file from one language to another.')
    parser.add_argument('-i', '--input', type=str, required=True,
                        help='The path to the text file to translate.')
    parser.add_argument('-o', '--output', type=str, required=True,
                        help='The path to save the translated text file to.')
    parser.add_argument('-s', '--source-language', type=str, required=True,
                        help='The language of the text file to translate.')
    parser.add_argument('-t', '--target-language', type=str, required=True,
    ...
    parser.add_argument('-c', '--checkpoint', type=str, default='translator.pkl',
    ...
    translated_text = translate(text, args.source_language, args.target_language,
                                args.model, args.checkpoint, verbose=args.verbose)
